...
def _plot2d(vi, X, target='vanishing', n=1000, scale=1.5, x_max=1.0, y_max=1.0, show=False, splitshow=False):
    ...
    y = torch.linspace(-scale*y_max, scale*y_max, 50)
    Z1, Z2 = torch.meshgrid(x, y)
    ...
    colors = plt.cm.Dark2(np.linspace(0,1,8))   # use np not torch
    linestyles = ['solid','dashed','dashdot', 'dotted']
    nfigs = min(npolys, n)

    for i in range(nfigs):
        f = lambda x_, y_: vi.evaluate(torch.tensor([[x_,y_]]), target=target)[0,i]
        f = np.vectorize(f)
