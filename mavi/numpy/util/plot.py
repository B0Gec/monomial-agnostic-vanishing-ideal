...
def _plot2d_ineq(vi, X, target='vanishing', n=1000, scale=1.5, x_max=1.0, y_max=1.0, show=False, splitshow=False):
    ...
    for i in range(nfigs):
        ...
        f = np.vectorize(f)
        plt.contour(Z1,Z2,f(Z1, Z2), levels=[0], colors=[colors[i%len(colors)]], linewidths=[1.], linestyles=[linestyles[i%4]])
        plt.contourf(Z1,Z2,(f(Z1, Z2) <= 0).astype('float'), levels=[0], alpha=0.1)
